# Race and Surveillance Technology in K-12 Education Lab 

<a href="http://creativecommons.org/licenses/by-nc/4.0/" rel="license"><img style="border-width: 0;" src="https://i.creativecommons.org/l/by-nc/4.0/88x31.png" alt="Creative Commons License" /></a>
This tutorial is licensed under a <a href="http://creativecommons.org/licenses/by-nc/4.0/" rel="license">Creative Commons Attribution-NonCommercial 4.0 International License</a>.

## Table of Contents

- [Lab Materials](#lab-materials)
- [Background](#background)
- [Lab Options](#lab-options)
  * [Prompt #1: School Policies/Regulations](#prompt-1-school-policiesregulations)
  * [Prompt #2: Tech Deep Dive (Company or Product)](#prompt-2-tech-deep-dive-company-or-product)
  * [Prompt #3: Surveillance Tech in Higher Education](#prompt-3-surveillance-tech-in-higher-education)
  * [Prompt #4: Further Background Research](#prompt-4-further-background-research)
- [Lab Notebook Components](#lab-notebook-components)


# Lab Materials

[Lab notebook template](https://docs.google.com/document/d/16Gu3MKXMz2FCqcxXXeSmiz1tQGTP62cdtsZsoHpVWJY/copy) (Google Doc)

# Background

For this lab, we read a selection of articles that focus on the intersection of race and surveillance in K-12 education.

- Claire Galligan, Hannah Rosenfeld, Molly Kleinman, Shobita Parthasarathy, “[Cameras in the Classroom: Facial Recognition Technology in Schools](https://stpp.fordschool.umich.edu/research/research-report/cameras-classroom-facial-recognition-technology-schools)” *University of Michigan’s Science, Technology, and Public Policy Program* (25 August 2020)

- Jack Gillum and Jeff Kao, with photography by Adrienne Grunwald, “[Aggression Detectors: The Unproven, Invasive Surveillance Technology Schools Are Using to Monitor Students](https://features.propublica.org/aggression-detector/the-unproven-invasive-surveillance-technology-schools-are-using-to-monitor-students/)” *ProPublica* (25 June 2019)

- Jennifer Smith Richards, Jodi S. Cohen, and Lakeidra Chavis, with photography by Zbigniew Bzdak, “[The Quiet Rooms: Children are being locked away, alone and terrified, in schools across Illinois. Often, it’s against the law](https://features.propublica.org/illinois-seclusion-rooms/school-students-put-in-isolated-timeouts/)” *ProPublica Illinois* (19 November 2019)
  * [More reporting from this investigation, a collaboration between ProPublica Illinois and the Chicago Tribune](https://www.propublica.org/series/illinois-school-seclusions-timeouts-restraints)

- Coshandra Dillard, “[Envisioning School Safety Without Police](https://www.learningforjustice.org/magazine/fall-2021/envisioning-school-safety-without-police)” *Learning for Justice* (Fall 2021)

We'll start by focusing on the "Cameras in the Classroom" report. A few discussion questions, or places to start:
- What stood out to you when reading the executive summary? 
- What were able to tell about the scale or scope of facial recognition tech in schools? 
  * Number of schools, types of technology, number of cameras, number of impacted students, etc.
- What stood out to you from the report's organization or structure? Takeaways from particular sections you focused on or read in detail?
- Connections with previous class content/discussions
- Other initial questions/comments/observations

Our readings for this week challenge us to think expansively about what surveillance looks like and how it shows up particularly in K12 education. Thinking across these readings...
- What stood out to you in terms of how these articles present or discuss surveillance? Anything that was particularly challenging, surprising, eye-opening, etc?
- Where and how the intersection of race and surveillance shows up
- What kinds of "othering" happens via surveillance technologies in education, thinking about race as well as gender, sexuality, ability and disability, neurotypicality and neurodivergence, socio-economic class, cultural and linguistic identity, religious practice, etc.
- Connections with previous class content/discussions
- Other questions/comments/observations

Some of our content for this week- specifically the "Cameras in the Classroom" report and Coshandra Dillard's *Learning for Justice* article- make specific policy recommendations or ask us to imagine alternative frameworks and possibilities for surveillance in K12 education.
- What stood out to you from this article
- Connections with other readings from this week and/or previous class discussions
- Other questions/comments/observations

# Lab Options

The lab procedure document includes four (4) different prompt options. Folks can go in-depth with one prompt or explore a couple of prompts. Collaboration is encouraged.

## Prompt 1: School Policies/Regulations

As we see across this week's readings, policies around racialized surveillance tech (facial recognition, SROs, etc) are often highly localized and coordinated at the district/community level.

For example, the South Bend Community School Corporation has been having an ongoing conversation around the role of SROs leading up to the approval of an agreement in December 2021. Sources of related information include...
- [SBCSC website](http://sb.school/)
- [SBCSC School Board Documents](https://go.boarddocs.com/in/sbcsc/Board.nsf/Public)
- [South Bend Tribune](https://www.southbendtribune.com/) (weekly paper)
- [NDSMC Observer](https://ndsmcobserver.com/2021/10/south-bend-community-advocates-against-police-officers-in-schools/) (student paper)
- [WSBT](https://wsbt.com/) (local CBS affiliate)
- [WNDU](https://www.wndu.com/) (local NBC affiliate)
- [ABC57](https://www.abc57.com/) (local ABC affiliate)
- [South Bend Police Department statement](https://police.southbendin.gov/press-release/clearing-sbpd-school-resource-officer-misinformation)

Another example includes a database of district policies and news stories for Iowa schools.
- [Link to database](https://airtable.com/shrcAos4QdCkA0BC7)
- [Link to more background on this project](https://github.com/kwaldenphd/S2PP-Data#qualitative-data)

Select a specific community or district and explore local news coverage, school/district websites, community organizations (PTO, BLM, etc).
- What surveillance tech is in use (or is under consideration)?
- What policies or regulations exist (or are under consideration)?
- Who are the stakeholders, and has their been any conversation/dialogue/pushback around the adoption of existing or proposed surveillance tech?
- Where and how can we see race and surveillance at work?
  * Connections with this week's readings and/or previous course content?
- Other observations/comments/questions

## Prompt 2: Tech Deep Dive (Company or Product)

Our content for this week mentions different types of surveillance technology used in schools as well as specific companies and products. Select a specific type of technology, company, or product, and do a deep dive using the question prompts below as a starting point.
- NOTE: You'll need to do some information gathering here. Wikipedia and/or the company website are places to start. K12 education trade publications can also be useful ([EducationWeek](https://www.edweek.org/), [EdTech Magazine](https://edtechmagazine.com/k12/), [TEACH Magazine](https://teachmag.com/), etc).

A few places to start...

Sample Categories:
- Metal detectors
- Facial recognition
- School resource officers
- Locker searches
- Drug testing
- Exam proctoring
- Learning management systems (Google Classroom, Canvas, Brightspace, etc)

Sample Companies:
- [Gaggle](https://www.gaggle.net/who-we-are)
- [Raptor Technologies](https://raptortech.com/)
- [School Innovations and Achievement](https://www.sia-us.com/)
- [Impero](https://www.imperosoftware.com/education/)
- [LanSchool](www.lanschool.com)
- [Navigate360](https://navigate360.com/who-we-serve/schools-k-12)
- [SchoolPass](https://schoolpass.com/)

Sample examples of specific learning management systems (one type of tech product- many of these are also companies):
- [Google Classroom](https://en.wikipedia.org/wiki/Google_Classroom)
- [Blackboard](https://www.blackboard.com/industries/k-12)
- [Desire 2 Learn](https://www.d2l.com/PRODUCTS/#PLATFORM)
- [Canvas](https://www.instructure.com/k-12)
- [itslearning](https://itslearning.com/global/schools/)
- [Otus](https://otus.com/)
- [PowerSchool](https://www.powerschool.com/)
- [Schoology](https://www.schoology.com/k-12)

Select one of these categories, tech programs, or tech companies.

If looking at a category of tech or specific program:
- How (and why) was this technology developed?
- Who all was involved, or where did it originate?
- What can you tell about how the technology works?
  * Anything you can tell about data privacy/security, required tech, or price point/pricing model?
- Where is this technology used?
- Where and how can we see race and surveillance at work?
  * Connections with this week's readings and/or previous course content?
- Other observations/comments/questions

If looking at a specific company:
- How (and why) does this company exist?
- What can you tell about their origin story and leadership structure?
- What product(s) do they develop and where are they used?
  * Anything you can tell about required tech or price point/pricing model?
- Connections with this week's readings and/or previous course content?
- Other observations/questions/comments

## Prompt 3: Surveillance Tech in Higher Education

The kinds of surveillance technologies we're looking at this week aren't unique to K12 education- they also show up on college campuses in a variety of contexts.

OIT maintains [a Google Sheet](https://nd.service-now.com/nd_portal?id=kb_article_view&sysparm_article=KB0014445&sys_kb_id=4192330bdb010150310fa94ed396195f&spa=1) with software titles that have been approved for use at ND.

Some tools that might show up in ND classes:
- PollEverywhere
- Canvas
- Google Classroom
- Sakai
- Perusall
- Piazza
- Panopto
- Gradescope
- Kahoot

In some cases, folks can download their user data from these platforms.
- Let Prof. Walden know (via email) if you want your Canvas data/analytics for this course.
- OIT will likely ask questions, but you could [submit a ticket](https://nd.service-now.com/nd_portal) requesting your data/analytics for a specific enterprise tool or platform (Canvas, Panopto, etc).
  * With the disclaimer I am not a lawyer, access to this information might even be your legal right under FERPA: [link to more info](https://studentprivacy.ed.gov/faq/how-long-does-educational-agency-or-institution-have-comply-request-view-records).

If looking at a specific company or product:
- How (and why) was this technology developed?
- Who all was involved, or where did it originate?
- What can you tell about how the technology works?
  * Anything you can tell about data privacy/security, required tech, or price point/pricing model?
- Where is this technology used?
- Where and how can we see race and surveillance at work?
  * Connections with this week's readings and/or previous course content?
- Other observations/comments/questions

Some resources that might be helpful if looking at your own user data for one of these platforms:
- Exploratory data analysis in Tableau
  * Tableau Public, ["How-To Videos and Resources"](https://public.tableau.com/en-us/s/resources)
  * Miriam Posner, ["Getting started with Tableau Public"](http://miriamposner.com/classes/dh201w19/tutorials-guides/data-visualization/getting-started-with-tableau-public/) tutorial for Winter 2019 "Digital Humanities 201" class
- Exploratory data analysis in a spreadsheet program ([Excel](https://support.microsoft.com/en-us/office/create-a-chart-from-start-to-finish-0baf399e-dd61-4e18-8a73-b3fd5d5680c2), [Google Sheets](https://support.google.com/docs/answer/190718?hl=en))
- [Exploratory data analysis in WTFcsv](https://github.com/kwaldenphd/football-structured-data/blob/main/eda.md#databasic-wtfcsv)
- Folks are also welcome to explore this data in Python, RStudio, etc.

## Prompt 4: Further Background Research

There's significant additional public-facing writing, research, and advocacy at the intersection of race, surveillance, and education.

Select one of the larger reports ("Cameras in the Classroom", one of the ACLU reports, one of the Georgetown reports) or a few of the articles and do a deep dive.

### Reports

- Claire Galligan, Hannah Rosenfeld, Molly Kleinman, Shobita Parthasarathy, “[Cameras in the Classroom: Facial Recognition Technology in Schools](https://stpp.fordschool.umich.edu/research/research-report/cameras-classroom-facial-recognition-technology-schools)” *University of Michigan’s Science, Technology, and Public Policy Program* (25 August 2020)

- Georgetown Law’s Center on Poverty and Inequality has an Initiative on Gender Justice and Opportunity doing [ongoing research on education and pushout for girls of color](https://genderjusticeandopportunity.georgetown.edu/school-and-pushout/)

- American Civil Liberties Union, et al, “[The Cost of School Policing: What Florida’s students have paid for a pretense of security](https://www.aclufl.org/en/publications/cost-school-policing)” *ACLU Florida* (2 September 2020)

- Two-part ACLU report:
  * UCLA Center for Civil Rights Remedies and ACLU of Southern California, “[11 Million Days Lost: Race, Discipline, and Safety and U.S. Public Schools](https://www.aclu.org/report/11-million-days-lost-race-discipline-and-safety-us-public-schools-part-1)” *ACLU* (24 August 2018)
  * ACLU et al, “[Cops and no Counselors: How the Lack of School Mental Health Staff is Harming Students](https://www.aclu.org/report/cops-and-no-counselors)” *ACLU* (4 May 2019)

### Investigative Journalism

- Meribah Knight and Ken Armstrong, “[Black Children Were Jailed for a Crime That Doesn’t Exist. Almost Nothing Happened to the Adults in Charge](https://www.propublica.org/article/black-children-were-jailed-for-a-crime-that-doesnt-exist)” *ProPublica* (8 October 2021)

- Colin Lecher and Maddy Varner, “[NYC’s School Algorithms Cement Segregation. This Data Shows How](https://themarkup.org/news/2021/05/26/nycs-school-algorithms-cement-segregation-this-data-shows-how)” *Markup* (26 May 2021)

- Mark Keierleber, “[Gaggle Surveils Millions of Kids in the Name of Safety. Targeted Families Argue it’s ‘Not That Smart’](https://www.the74million.org/article/gaggle-surveillance-minnesapolis-families-not-smart-ai-monitoring/)” *74 Million* (12 October 2021)

- Heather Schlitz, “[Schools often use AI to find students in crisis - one software-monitoring company reported 5,000 self-harm situations in a single week](https://www.businessinsider.com/school-students-communication-monitoring-ai-software-self-harm-suicide-risks-2021-9)” *Insider* (21 September 2021)

### Higher Education/Academic Research

- *Journal of Interactive Technology and Pedagogy* has a [special issue on surveillance and educational technology](https://jitp.commons.gc.cuny.edu/table-of-contents-issue-twenty/) (December 2021), co-edited by Chris Gilliard (author of article we read in week #1)

### Discussion Questions/Prompts

If looking at a research-based report or whitepaper:
- What report(s) did you look at?
- What was the goal, scope, or central questions?
- What data sources did they draw on (and why)?
  * Or, what types of information did they bring together?
- What kinds of research/analysis methods did they use?
  * Specific questions, Statistical methods, Visualizations/calculations, etc
- What conclusions do the authors draw from the analysis?
- Connections we can make with race and surveillance?
  * Connections with previous course content
- Other thoughts/comments/questions/observations.

If looking at investigative journalism work:
- What story/stories did you focus on?
- What stood out?
- Where and how do we see race and surveillance at work?
  * Connections with previous course content
- What are you able to tell about how the authors' analyzed or approached/framed surveillance?
  * How do the authors describe the role/functionality/design/etc. of the surveillance system?
  * Whose perspectives or voices are highlighted?
- Other thoughts/comments/questions/observations.

# Lab Notebook Components

[Lab notebook template](https://docs.google.com/document/d/16Gu3MKXMz2FCqcxXXeSmiz1tQGTP62cdtsZsoHpVWJY/copy) (Google Doc)

The lab notebook consists of a narrative that documents and describes your experience working through this lab.

Folks can take notes on the reading-based questions and then focus on the questions/topics for the specific prompt you choose. 

If working collaboratively, reflection should include comments from each member of the group. These can be combined in a single notebook or submitted individually along with work completed collaboratively.
